...
<body>
    ...
    <nav class="navbar-custom">
      <ul class="list-unstyled topbar-nav float-right mb-0">
        ...
        <li class="ses" style="display: flex !important;align-items: center !important; margin-top: -3px !important;">
          <!-- <a href="#" style="font-size: 1em !important; color: #1761fd !important" class="mdi mdi-plus-circle-outline" aria-hidden="true" data-toggle="modal" -->
          <!-- data-target="#exampleModal"></a> -->
          <a href="#" style="font-size: 0.85em !important; color: #1761fd !important"
            class="mdi mdi-plus-circle-outline" onclick="initAdd()" aria-hidden="true"></a>
        </li>
    ...
    <ul class="metismenu left-sidenav-menu">
      <li>
        ...
        <ul class="nav-second-level" aria-expanded="false">
          ...
          <li class="nav-item"><a class="nav-link" href="../dashboard/crm-index.php"><i
                class="ti-control-record"></i>CRM</a></li>
          <li class="nav-item"><a class="nav-link" href="../dashboard/helpdesk-index.php"><i
                class="ti-control-record"></i>Helpdesk</a></li>
          <li class="nav-item"><a class="nav-link" href="../dashboard/sales-index.php"><i
        ...
        <div class="row">
          <div class="col-sm-12">
            <div class="page-title-box">
              ...
              <h4 class="page-title">Fabric List</h4>
            </div>
          </div>
        </div>
        <div class="row4">
  ...
  <select id='simple_select' class='form-control' data-toggle='select' title='Simple select' data-live-search='true'
    data-live-search-placeholder='Search ...' style="display: none" onchange="myFunction()"></select>
  <select id='yarn_select' class='form-control cust' data-toggle='select' title='Simple select' data-live-search='true'
    data-live-search-placeholder='Search ...' style="display: none">
    ...
    $(document).ready(function() {
      fetch('customers_connect.php').then(result => result.json()).then(data => {
        let cus = '<option></option>';
        data.map(customer => {
          cus += "<option>" + customer.first_name + " " + customer.last_name + "</option>";
        });
        return cus;
      }).then(datata => $("#simple_select").append(datata));
    });
    </script>
    ...
    function myFunction() {
      $("#yarn_select").html("");
      fetch('yarn_connect.php', {
        method: 'POST',
        body: JSON.stringify(document.getElementById('simple_select').options[
        ...
        let dupl = [];
        data.map(customer => {
          dupl.push(customer.yarn_type);
        });
        let newDupl = Array.from(new Set(dupl));
        newDupl = newDupl.filter(element => element !== "");
        console.log(newDupl);
        newDupl.map(customer => {
          cus += "<option>" + customer + "</option>";
        });
        return cus;
      }).then(datata => {
        // console.log(datata);
        $("#yarn_select").append(datata)
      });
    }
    </script>
    ...
    <!-- <script>
  $(document).ready(function() {
    $('#simple_select').addEventListener('select', () => {
      console.log(
        document.getElementById('simple_select').options[
          document.getElementById('simple_select').selectedIndex
        ].value
      );
    });
    ...
    $(document).ready(function() {
      ...
      $($($("#datatable-basic_wrapper").children()[1])[0].childNodes[0]).css({
        "padding-right": "0px",
        "padding-left": "0px",
        "margin-left": "-1px"
      });
